...
#pragma region STATE JUMP
		...
		{
			if ((keyboard->GetKeyStateDown(DIK_SPACE) && isFly == false) || bounceAfterJumpOnEnemy == true)
			{
				float jumpMaxHeight;
...
void CMario::HoldProcess()
{
	...
	{
		if (keyboard->GetKeyStateDown(DIK_LCONTROL) == true) // Vẫn đang ấn giữ phím A
		{
		...
		switch (dieState)
		{
			...
			case 2:
			{
				...
				if (transform.position.y >= cam->GetCurrentBoundary().bottom - DIE_FALL)
				{
					...
					sceneManager->SwitchScene(new CScene1);
					isAutogo = false;
...
{
	// EVENT
	if ((KeyCode == DIK_SPACE || KeyCode == DIK_X) && isOnGround == true && currentPhysicsState.jump == JumpOnAirStates::Stand)
	{
		// JUMP
		...
		if (currentPhysicsState.jump == JumpOnAirStates::Stand && KeyCode == DIK_SPACE)
			currentPhysicsState.jump = JumpOnAirStates::Jump;
	}
